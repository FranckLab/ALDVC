...
mex -O ba_interp3.cpp; warning('off'); % dbstop if error % % Old version codes.
...
DVCpara.ICGNtol=1e-2; % IC-GN stopping threshold
...
for ImgSeqNum = 2:length(ImgNormalized)
    ...
    % Df = funImgGradient3(Img{1},'stencil7'); Df.imgSize = size(Img{1});
    ...
    [USubpb1,FSubpb1,HtempPar,ALSub1Timetemp,ConvItPerEletemp] = ...
        LocalICGN3(U0,DVCmesh.coordinatesFEM,Df,Img{1},Img{2},DVCpara,'GaussNewton',DVCpara.ICGNtol);
    ALSub1Time(ALSolveStep) = ALSub1Timetemp; 
    ConvItPerEle(:,ALSolveStep) = ConvItPerEletemp; toc
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
        ...
        MNL = size(DVCmesh.xyz0); GaussPtOrder = 2; alpha = 0;
        close all; hbar = waitbar(0,'Please wait for Subproblem 2 global step!'); 
        % ====== Solver using finite element method ======
        for tempk = 1:length(betaList)
            beta = betaList(tempk);
            GaussPtOrder = 2; alpha = 0; [USubpb2] = Subpb23(DVCmesh,beta,mu,USubpb1,FSubpb1,udual,vdual,alpha,GaussPtOrder);
    ...
    % Plotdisp03(USubpb1,DVCmesh.coordinatesFEM,DVCmesh.elementsFEM,'All');
    % Plotstrain03(full(FLocalICGN),xyz0.x,xyz0.y,xyz0.z,size(Img{1}),'All');
    ...
    while (ALSolveStep < 4)
        ...
        Df,Img{1},Img{2},mu,beta,HPar,ALSolveStep,DVCpara,'GaussNewton',DVCpara.ICGNtol);
        FSubpb1 = FSubpb2; toc
        ALSub1Time(ALSolveStep) = ALSub1Timetemp; ConvItPerEle(:,ALSolveStep) = ConvItPerEletemp; 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % ------  Manually find some bad points from Local Subset ICGN step ------
        % Comment START
